/* GENERATED FILE */
const files = {}
files["events"] = require("@factor/plugin-events").default
files["tools"] = require("@factor/plugin-tools").default
files["auth"] = require("@factor/plugin-auth").default
files["blog"] = require("@factor/plugin-blog").default
files["db"] = require("@factor/plugin-db").default
files["directivesstandard"] = require("@factor/plugin-directives-standard").default
files["googleapi"] = require("@factor/plugin-google-api").default
files["markdown"] = require("@factor/plugin-markdown").default
files["notify"] = require("@factor/plugin-notify").default
files["posts"] = require("@factor/plugin-posts").default
files["signin"] = require("@factor/plugin-signin").default
files["ssrprogressbar"] = require("@factor/plugin-ssr-progress-bar").default
files["time"] = require("@factor/plugin-time").default
files["user"] = require("@factor/plugin-user").default
files["src"] = require("@/plugin.js").default
module.exports = files